...
def render_planner_actions_block(
    ...
    *,
    ...
    with st.expander("Global Actions", expanded=False):
    ...
    run_completed = bool(m.get("execution_completed", False))
    write_mode_ui = st.session_state.get("execution_write_mode", "Overwrite (Truncate)")
    overwrite_clear_mode = "truncate_cascade"
    if str(write_mode_ui).startswith("Overwrite"):
        overwrite_clear_mode_ui = st.session_state.get(
            "overwrite_clear_mode_ui",
            "TRUNCATE ... CASCADE (recommended)",
        )
        overwrite_clear_mode = (
            "session_replica"
            if str(overwrite_clear_mode_ui).startswith("session_replication_role")
            else "truncate_cascade"
        )
    truncate_before_migration = bool(st.session_state.get("truncate_before_migration", True))
    ...
    with cols[0]:
        if st.button(
            ...
            mode = "overwrite" if str(write_mode_ui).startswith("Overwrite") else "append"
            render_execute_confirmation_dialog(
                db=db,
