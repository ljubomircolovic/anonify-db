...
def render_status_chain(all_tables, completed_tables):
    """Prikazuje lanac progresa na vrhu."""
    if all_tables:
...
def render_table_header_info(schema_name, table_name):
    """Prikazuje stilizovano zaglavlje trenutne tabele."""
    st.markdown(f"""
...
def render_ai_audit_log():
    """Prikazuje poslednji AI log ako postoji u sesiji."""
    if 'last_ai_audit' in st.session_state:
        with st.expander("📡 Poslednji AI Audit Log (Data Sent to Cloud)"):
